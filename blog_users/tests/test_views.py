...
class TestSignUpView(TestCase):
    def setUp(self):
        self.user = BlogUser.objects.create_user(username="user1", email="user1@gmail.com", password="1234")
        self.data = {
            "username": "test",
    ...
    def test_user_is_logged_in(self):
        response = self.client.post(reverse("blog_users:signup"), self.data, follow=True)
    ...
    def test_redirect_if_user_is_authenticated(self):
        login = self.client.login(email="user1@gmail.com", password="1234")  # noqa: F841
        response = self.client.get(reverse("blog_users:signup"))
        self.assertRedirects(response, reverse("blog_app:index"))

    def test_invalid_form(self):
        response = self.client.post(
            reverse("blog_users:signup"),
            {
...
class ProfileViewTest(TestCase):
    def setUp(self):
        self.user1 = BlogUser.objects.create_user(username="user1", email="user1@gmail.com", password="1234")
        self.user2 = BlogUser.objects.create_user(username="user2", email="user2@gmail.com", password="1234")

    def test_redirect_if_not_logged_in(self):
        response = self.client.get(reverse("blog_users:profile", kwargs=({"username": self.user1.username})))
        self.assertRedirects(response, "/blog_users/login/?next=/blog_users/profile/user1/")
    ...
    def test_view_returns_profile_of_a_given_user(self):
        self.client.login(email="user1@gmail.com", password="1234")
        response = self.client.get(reverse("blog_users:profile", kwargs=({"username": self.user2.username})))
...
class EditProfileViewTest(TestCase):
    ...
    def test_edit_profile_redirects_if_not_logged_in(self):
        response = self.client.get(reverse("blog_users:edit_profile"))
        self.assertRedirects(response, "/blog_users/login/?next=/blog_users/edit-profile/")

    def test_edit_profile_change_username(self):
        self.client.login(email="user1@gmail.com", password="1234")
        response = self.client.post(  # noqa: F841
            reverse("blog_users:edit_profile"), {"username": "user2", "about_me": "Hello world"}
        )
    ...
    def test_upload_image(self):
        ...
        with open(image.name, "rb") as f:
            response = self.client.post(  # noqa: F841
                reverse("blog_users:edit_profile"), {"username": "user1", "about_me": "Hello world", "image": f}
            )
